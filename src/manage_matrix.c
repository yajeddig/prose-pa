...
s_matrix_la *LA_matrix_calloc(int row, int col)
{
  int nr;
  s_matrix_la *matrix_alloc;

  matrix_alloc = new_matrix_la();
  matrix_alloc->row_size = row;
  matrix_alloc->col_size = col;

  matrix_alloc->data = (MATRIX_TYPE **) calloc(matrix_alloc->row_size, sizeof(MATRIX_TYPE *));
  
  for(nr = 0; nr < matrix_alloc->row_size; nr++)
     matrix_alloc->data[nr] = (MATRIX_TYPE *) calloc(matrix_alloc->col_size, sizeof(MATRIX_TYPE));
