...
#include <unistd.h>
#include <stdarg.h>
#include "param_PROSE.h"

char *PROSE_name_param(int param)
{
  char *name;

   switch(param) {
   case MAINT_DA : {name = strdup("maint");break;}
   case ALPHA_DA : {name = strdup("alpha");break;}
   case PMAX_DA : {name = strdup("pmax");break;}
   case ETA_CHLA_DA : {name = strdup("eta_chla");break;}
   case C_CHLA_DA : {name = strdup("c_chla");break;}
   case ETA_DA : {name = strdup("eta");break;}
   case TOPT_PHY_DA : {name = strdup("Topt_phy");break;}
   case MU_BACT_DA : {name = strdup("mu_max");break;}
   case Y_BACT_DA : {name = strdup("yield");break;}
   case MORT_BACT_DA : {name = strdup("mort_bact");break;}
   case TOPT_BACT_DA : {name = strdup("Topt_bact");break;}
   case KNAVIG_DA : {name = strdup("Knavig");break;}
   case B1_RIVER_DA : {name = strdup("b1_river");break;}     
   default : {name = strdup("DEFAULT, unknown parameters in itos.c");break;}
   }
